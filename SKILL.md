...
name: build
description: >
  Implement a feature from a plan — project setup, code structure, and systematic task-by-task coding. Use when setting up a project, writing implementation code, scaffolding, or building a feature. Triggered by: implement this, write the code, set up the project, scaffold, start coding.
---
...
## Working with AI Agents (AI agent)
...
- Use the terminal for running tests, git commands, and scripts.
- Keep `.env` in `.gitignore` — use environment secrets for environment variables in deployment.
- The your IDE file tree auto-saves — still commit frequently to preserve history.
- When using AI agent: give it one task at a time with clear context; review before moving on.
- Use your IDE's run command for the main entry point, Shell for everything else.
