...
name: tdd
description: >
  Write tests before writing implementation code, following Test-Driven Development (TDD).
  Use this skill whenever an engineer wants to write tests first, practice red-green-refactor,
  define behavior via tests before coding, write unit tests, write failing tests as a design
  tool, or use tests to drive the implementation of a function, module, class, or API endpoint.
  Trigger when the user says "write tests first", "TDD this", "help me test-drive this feature",
  "write failing tests for", "what should I test", or "help me define the behavior with tests".
  Use this skill in conjunction with the spec skill — acceptance criteria from specs map
  directly to test cases.
---
...
```
❌ Assert that bcrypt.hash() was called (testing HOW)
...
- Run tests in the Replit Shell tab with your framework's test command.
- Keep a terminal split open: one pane for editing, one for running tests continuously.
- When using Replit Agent to implement code, prefix your prompt with the test file and say
