...
name: test
description: >
  Write, organize, and run a comprehensive test suite for software — covering integration
  tests, end-to-end tests, regression tests, and test strategy. Use this skill when an engineer
  needs to test a completed feature, write integration or E2E tests, define a test strategy,
  set up a test environment, run a test suite, debug failing tests, ensure test coverage,
  or validate that software meets its requirements before shipping. Trigger when the user says
  "write integration tests", "write E2E tests", "test this feature end to end", "set up testing",
  "test strategy", "what should I test", "run tests", "debug failing tests", "test coverage",
  or "validate before shipping". This skill complements TDD (which focuses on unit tests first);
...
- For E2E tests in Replit: use a `.env.test` Replit Secret or a separate Repl for the test
  environment to keep it isolated from your dev environment.
- When using Replit Agent to write tests: provide the spec acceptance criteria and say
  "Write integration tests that verify these acceptance criteria."
- Replit's always-on feature can serve as a lightweight staging environment for E2E tests.
